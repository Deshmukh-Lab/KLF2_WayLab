...
#Make reference project UMAPs of the 3 datasets
#These are for the UMAPs seen in Extended Data 8 and 9 as seen in Seurat's vignette on reference mapping
...
#Map CD.jaeger data set to Ref 
anchors <- FindTransferAnchors(
  reference = CD.elemntaite,
  query = CD.jaeger,
...
CD.jaeger <- MapQuery(
  ...
  query = CD.jaeger,
  anchorset = anchors,
  refdata = list(celltype = "predicted.id"),
  reference.reduction = "pca",
...
p1 <- DimPlot(CD.jaeger, reduction = "ref.umap",group.by = "predicted.id", label = T) + NoLegend() + ggtitle("predicted.id")
p2 <- DimPlot(CD.jaeger, reduction = "ref.umap", group.by = "predicted.celltype", label = T) + NoLegend()+ ggtitle("predicted.celltype")
p3 <- DimPlot(CD.elemntaite, reduction = "ref.umap", label = T) + NoLegend()+ ggtitle("CD.elemntaite")
library(patchwork)
p1+p2+p3

#Map CD.matrin to Ref
...
p4 <- DimPlot(CD.martin, reduction = "ref.umap",group.by = "predicted.id", label = T) + NoLegend() + ggtitle("predicted.id")
p5 <- DimPlot(CD.martin, reduction = "ref.umap", group.by = "predicted.celltype", label = T) + NoLegend()+ ggtitle("predicted.celltype")
p3 <- DimPlot(CD.elemntaite, label = T) + NoLegend()+ ggtitle("CD.elemntaite")
library(patchwork)
p4+p5+p3
...
p6 <- DimPlot(UC, reduction = "ref.umap",group.by = "predicted.id", label = T) + NoLegend() + ggtitle("predicted.id")
p7 <- DimPlot(UC, reduction = "ref.umap", group.by = "predicted.celltype", label = T) + NoLegend()+ ggtitle("predicted.celltype")
p3 <- DimPlot(CD.elemntaite, label = T) + NoLegend()+ ggtitle("CD.elemntaite")
p6+p7+p3


#Finally Map CD.elemntatie data set to Ref 
anchors <- FindTransferAnchors(
  reference = CD.elemntaite,
  query = CD.elemntaite,
  reference.reduction = 'pca',
...
p9 <- DimPlot(CD.elemntaite, reduction = "ref.umap",group.by = "predicted.id", label = T) + NoLegend() + ggtitle("predicted.id")
p10 <- DimPlot(CD.elemntaite, reduction = "ref.umap", group.by = "predicted.celltype", label = T) + NoLegend()+ ggtitle("predicted.celltype")
p3 <- DimPlot(CD.elemntaite, reduction = "ref.umap", label = T) + NoLegend()+ ggtitle("CD.elemntaite")
library(patchwork)
p9+p10+p3


#Make figure with UMAP for all cells on same scales
A <- DimPlot(CD.jaeger, reduction = "ref.umap", group.by = "predicted.celltype", label = T, repel = T) + NoLegend()+ ggtitle("Jaeger et al") + xlim(-20,15) + ylim(-15,12) 
B <- DimPlot(CD.martin, reduction = "ref.umap", group.by = "predicted.celltype", label = T, repel = T) + NoLegend()+ ggtitle("Martin et al") + xlim(-20,15) + ylim(-15,12) 
C <- DimPlot(UC, reduction = "ref.umap", group.by = "predicted.celltype", label = T, repel = T) + NoLegend()+ ggtitle("Smillie et al") + xlim(-20,15) + ylim(-15,12)
D <- DimPlot(CD.elemntaite, reduction = "ref.umap", group.by = "predicted.celltype", label = T, repel = T) + NoLegend()+ ggtitle("Elementaite et al") + xlim(-20,15) + ylim(-15,12)
A+B # 1300*690
C+D

#Merged Figure 
All.datasets <- merge(CD.elemntaite, c(CD.jaeger,CD.martin,UC), merge.dr = "ref.umap")
DimPlot(All.datasets, reduction = "ref.umap", group.by = "predicted.id", label = T, repel = T) + NoLegend()+ ggtitle("All Data sets") + xlim(-20,15) + ylim(-15,12)
DimPlot(All.datasets, reduction = "ref.umap", group.by = "predicted.id", label = F, repel = T) + NoLegend()+ ggtitle("All Data sets") + xlim(-20,15) + ylim(-15,12)
DimPlot(All.datasets, reduction = "ref.umap", group.by = "dataset", label = T, repel = T, cols = c("Teichman" = "red", "Martin" ="blue", "UC"="grey", "Jaeger" = "green")) + ggtitle("All Data sets") + xlim(-20,15) + ylim(-15,12)
# 690x710

#Saved RDS 
saveRDS(All.datasets, file = "All.datasets.RDS")
#Read RDS
#All.datasets <- readRDS(file = "All.datasets.RDS")

head(All.datasets[[]])
Idents(All.datasets) <-"predicted.celltype"
levels(All.datasets)
table(All.datasets$predicted.celltype, All.datasets$dataset)

#Subset "CD4 T cell","Tfh","Treg"
All.datasets.T.cells <- subset(All.datasets, idents = c("CD4 T cell","Tfh","Treg"))
saveRDS(All.datasets.T.cells, file = "All.datasets.T.cells.RDS")

Idents(All.datasets.T.cells) <-"dataset"
T.cell.elementaite <- subset(All.datasets.T.cells, idents = "Teichmann")
T.cell.martin <- subset(All.datasets.T.cells, idents = "Martin")
T.cell.jaeger <- subset(All.datasets.T.cells, idents = "Jaeger")
T.cell.UC <- subset(All.datasets.T.cells, idents = "UC")

#690x710
DimPlot(All.datasets.T.cells, reduction = "ref.umap", group.by = "predicted.celltype", label = T, repel = T) + NoLegend()+ ggtitle("All Data sets") + xlim(-20,15) + ylim(-15,12)
d2 <- DimPlot(All.datasets.T.cells, reduction = "ref.umap", group.by = "dataset", label = F, repel = T, pt.size = 1) + ggtitle("All Data sets") + xlim(-20,15) + ylim(-15,12)
alpha.use <- 2/5
d2$layers[[1]]$mapping$alpha <- alpha.use
d3 <- d2 + scale_alpha_continuous(range = alpha.use, guide = F)
d3

d2 <- DimPlot(All.datasets.T.cells, reduction = "ref.umap", split.by = "dataset", label = F, repel = T, pt.size = 1) + ggtitle("All Data sets") + xlim(-20,15) + ylim(-15,12)
alpha.use <- 2/5
d2$layers[[1]]$mapping$alpha <- alpha.use
d3 <- d2 + scale_alpha_continuous(range = alpha.use, guide = F)
d3
...
E <- DimPlot(T.cell.jaeger, reduction = "ref.umap", group.by = "predicted.celltype", label = T, repel = T) + NoLegend()+ ggtitle("Jaeger et al") + xlim(-20,15) + ylim(-15,12) 
F <- DimPlot(T.cell.martin, reduction = "ref.umap", group.by = "predicted.celltype", label = T, repel = T) + NoLegend()+ ggtitle("Martin et al") + xlim(-20,15) + ylim(-15,12) 
G <- DimPlot(T.cell.UC, reduction = "ref.umap", group.by = "predicted.celltype", label = T, repel = T) + NoLegend()+ ggtitle("Smillie et al") + xlim(-20,15) + ylim(-15,12)
H <- DimPlot(T.cell.elementaite, reduction = "ref.umap", group.by = "predicted.celltype", label = T, repel = T) + NoLegend()+ ggtitle("Elementaite et al") + xlim(-20,15) + ylim(-15,12)

E+F
G+H
#1300 X690

#All CD data sets (merged) and then single
Idents(All.datasets.T.cells) <-"dataset"
CD.all <- subset(All.datasets.T.cells, idents = "UC", invert = T)
UC.all <- subset(All.datasets.T.cells, idents = "UC")

#First for Crohn's disease
head(CD.all[[]])
Idents(CD.all) <-"health_meta"
levels(CD.all)
#"Teichmann- Crohn Disease"  "Teichmann- Normal control" "Jaeger-CD"                 "Jaeger-HC"                 "Martin-inflammed"  
CD.all.HC <- subset(CD.all, idents = c("Teichmann- Normal control", "Jaeger-HC"))
CD.all.CD <- subset(CD.all, idents = c("Teichmann- Crohn Disease", "Jaeger-CD", "Martin-inflammed"))

A1 <- FeaturePlot(CD.all.HC, features = c("KLF2"), pt.size = 1, cols = c("grey","red")) + ggtitle("KLF2 Healthy Control") + xlim(-20,15) + ylim(-15,12)
A2 <- FeaturePlot(CD.all.HC, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4")) + ggtitle("FOXP3 Healthy Control") + xlim(-20,15) + ylim(-15,12)
A3 <- FeaturePlot(CD.all.CD, features = c("KLF2"), pt.size = 1, cols = c("grey","red")) + ggtitle("KLF2 Crohn's Disease") + xlim(-20,15) + ylim(-15,12)
A4 <- FeaturePlot(CD.all.CD, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4")) + ggtitle("FOXP3 Crohn's Disease") + xlim(-20,15) + ylim(-15,12)

A1+A2 #470X800
A3+A4


#for individual cells now. 
head(CD.all.HC[[]])
Idents(CD.all.HC) <- "predicted.celltype"
Idents(CD.all.CD) <- "predicted.celltype"
levels(CD.all.HC)
levels(CD.all.CD)
#[1] "Treg"       "CD4 T cell" "Tfh"   

#First for all Crohn's disease data sets for Healthy Controls
CD.all.HC.CD4 <- subset(CD.all.HC, idents = "CD4 T cell")
A11 <- FeaturePlot(CD.all.HC.CD4, features = c("KLF2"), pt.size = 1, cols = c("grey","red")) + ggtitle("KLF2 expression in CD4 T cells in Control") + xlim(-20,15) + ylim(-15,12)
A12 <- FeaturePlot(CD.all.HC.CD4, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4")) + ggtitle("FOXP3 expression in CD4 T cells in Control") + xlim(-20,15) + ylim(-15,12)
A11+A12

CD.all.HC.Tfh <- subset(CD.all.HC, idents = "Tfh")
A21 <- FeaturePlot(CD.all.HC.Tfh, features = c("KLF2"), pt.size = 1, cols = c("grey","red")) + ggtitle("KLF2 expression in Tfh cells in Control") + xlim(-20,15) + ylim(-15,12)
A22 <- FeaturePlot(CD.all.HC.Tfh, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4")) + ggtitle("FOXP3 expression in Tfh cells in Control") + xlim(-20,15) + ylim(-15,12)
A21+A22

CD.all.HC.Treg <- subset(CD.all.HC, idents = "Treg")
A31 <- FeaturePlot(CD.all.HC.Treg, features = c("KLF2"), pt.size = 1, cols = c("grey","red")) + ggtitle("KLF2 expression in Tregs in Control") + xlim(-20,15) + ylim(-15,12)
A32 <- FeaturePlot(CD.all.HC.Treg, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4")) + ggtitle("FOXP3 expression in Tregs in Control") + xlim(-20,15) + ylim(-15,12)
A31+A32 #470x800

#Next for all Crohn's disease data sets for Chron's Disease
CD.all.CD.CD4 <- subset(CD.all.CD, idents = "CD4 T cell")
A41 <- FeaturePlot(CD.all.CD.CD4, features = c("KLF2"), pt.size = 1, cols = c("grey","red")) + ggtitle("KLF2 expression in CD4 T cells in Crohn's disease") + xlim(-20,15) + ylim(-15,12)
A42 <- FeaturePlot(CD.all.CD.CD4, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4")) + ggtitle("FOXP3 expression in CD4 T cells in Crohn's disease") + xlim(-20,15) + ylim(-15,12)
A41+A42

CD.all.CD.Tfh <- subset(CD.all.CD, idents = "Tfh")
A51 <- FeaturePlot(CD.all.CD.Tfh, features = c("KLF2"), pt.size = 1, cols = c("grey","red")) + ggtitle("KLF2 expression in Tfh in Crohn's Disease") + xlim(-20,15) + ylim(-15,12)
A52 <- FeaturePlot(CD.all.CD.Tfh, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4")) + ggtitle("FOXP3 expression in Tfh cells in Crohn's Disease") + xlim(-20,15) + ylim(-15,12)
A51+A52

CD.all.CD.Treg <- subset(CD.all.CD, idents = "Treg")
A61 <- FeaturePlot(CD.all.CD.Treg, features = c("KLF2"), pt.size = 1, cols = c("grey","red")) + ggtitle("KLF2 expression in Tregs in Control") + xlim(-20,15) + ylim(-15,12)
A62 <- FeaturePlot(CD.all.CD.Treg, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4")) + ggtitle("FOXP3 expression in Tregs in Control") + xlim(-20,15) + ylim(-15,12)
A61+A62 #470x800



#For all T cells
FeaturePlot(CD.all.HC, features = c("KLF2"), pt.size = 1, cols = c("grey","red"), split.by = "predicted.celltype")  
FeaturePlot(CD.all.HC, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4"), split.by = "predicted.celltype") + xlim(-20,15) + ylim(-15,12)
FeaturePlot(CD.all.CD, features = c("KLF2"), pt.size = 1, cols = c("grey","red"), split.by = "predicted.celltype") + xlim(-20,15) + ylim(-15,12) # 1250 x 400
FeaturePlot(CD.all.CD, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4"), split.by = "predicted.celltype") + xlim(-20,15) + ylim(-15,12)


#Now for UC 
Idents(UC.all) <-"health_meta"
levels(UC.all)
#"UC-Inflamed" "UC-Healthy" 
UC.all.HC <- subset(UC.all, idents = "UC-Healthy")
UC.all.UC <- subset(UC.all, idents = "UC-Inflamed")

A5 <- FeaturePlot(UC.all.HC, features = c("KLF2"), pt.size = 1, cols = c("grey","red")) + ggtitle("KLF2 Healthy Control") + xlim(-20,15) + ylim(-15,12)
A6 <- FeaturePlot(UC.all.HC, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4")) + ggtitle("FOXP3 Healthy Control") + xlim(-20,15) + ylim(-15,12)
A7 <- FeaturePlot(UC.all.UC, features = c("KLF2"), pt.size = 1, cols = c("grey","red")) + ggtitle("KLF2 Ulcerative Colitis") + xlim(-20,15) + ylim(-15,12)
A8 <- FeaturePlot(UC.all.UC, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4")) + ggtitle("FOXP3 Ulcerative Colitis") + xlim(-20,15) + ylim(-15,12)

A5+A6 #470X800
A7+A8

#Now for all Ulceratice colitis data sets
Idents(UC.all.UC) <- "predicted.celltype"
Idents(UC.all.HC) <- "predicted.celltype"


#Nextfor all Ulcerative colitis data sets for Healthy Controls
UC.all.HC.CD4 <- subset(UC.all.HC, idents = "CD4 T cell")
B11 <- FeaturePlot(UC.all.HC.CD4, features = c("KLF2"), pt.size = 1, cols = c("grey","red")) + ggtitle("KLF2 expression in CD4 T cells in Control UC") + xlim(-20,15) + ylim(-15,12)
B12 <- FeaturePlot(UC.all.HC.CD4, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4")) + ggtitle("FOXP3 expression in CD4 T cells in Control UC") + xlim(-20,15) + ylim(-15,12)
B11+B12

UC.all.HC.Tfh <- subset(UC.all.HC, idents = "Tfh")
B21 <- FeaturePlot(UC.all.HC.Tfh, features = c("KLF2"), pt.size = 1, cols = c("grey","red")) + ggtitle("KLF2 expression in Tfh cells in Control UC") + xlim(-20,15) + ylim(-15,12)
B22 <- FeaturePlot(UC.all.HC.Tfh, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4")) + ggtitle("FOXP3 expression in Tfh cells in Control UC") + xlim(-20,15) + ylim(-15,12)
B21+B22

UC.all.HC.Treg <- subset(UC.all.HC, idents = "Treg")
A31 <- FeaturePlot(UC.all.HC.Treg, features = c("KLF2"), pt.size = 1, cols = c("grey","red")) + ggtitle("KLF2 expression in Tregs in Control UC") + xlim(-20,15) + ylim(-15,12)
A32 <- FeaturePlot(UC.all.HC.Treg, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4")) + ggtitle("FOXP3 expression in Tregs in Control UC") + xlim(-20,15) + ylim(-15,12)
A31+A32 #470x800

#Next for all lcerative colitis data sets for Ulcerative colitis
UC.all.UC.CD4 <- subset(UC.all.UC, idents = "CD4 T cell")
B41 <- FeaturePlot(UC.all.UC.CD4, features = c("KLF2"), pt.size = 1, cols = c("grey","red")) + ggtitle("KLF2 expression in CD4 T cells in Ulcerative colitis") + xlim(-20,15) + ylim(-15,12)
B42 <- FeaturePlot(UC.all.UC.CD4, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4")) + ggtitle("FOXP3 expression in CD4 T cells in Ulcerative colitis") + xlim(-20,15) + ylim(-15,12)
B41+B42

UC.all.UC.Tfh <- subset(UC.all.UC, idents = "Tfh")
B51 <- FeaturePlot(UC.all.UC.Tfh, features = c("KLF2"), pt.size = 1, cols = c("grey","red")) + ggtitle("KLF2 expression in Tfh in Ulcerative colitis") + xlim(-20,15) + ylim(-15,12)
B52 <- FeaturePlot(UC.all.UC.Tfh, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4")) + ggtitle("FOXP3 expression in Tfh cells in Ulcerative colitis") + xlim(-20,15) + ylim(-15,12)
B51+B52

UC.all.UC.Treg <- subset(UC.all.UC, idents = "Treg")
B61 <- FeaturePlot(UC.all.UC.Treg, features = c("KLF2"), pt.size = 1, cols = c("grey","red")) + ggtitle("KLF2 expression in Tregs in Ulcerative colitis") + xlim(-20,15) + ylim(-15,12)
B62 <- FeaturePlot(UC.all.UC.Treg, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4")) + ggtitle("FOXP3 expression in Tregs in Ulcerative colitisl") + xlim(-20,15) + ylim(-15,12)
B61+B62 #470x800


#UC plots for all cells.

FeaturePlot(UC.all.HC, features = c("KLF2"), pt.size = 1, cols = c("grey","red"), split.by = "predicted.celltype") # 1250 x 400
FeaturePlot(UC.all.HC, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4"), split.by = "predicted.celltype")
FeaturePlot(UC.all.UC, features = c("KLF2"), pt.size = 1, cols = c("grey","red"), split.by = "predicted.celltype") # 1250 x 400
FeaturePlot(UC.all.UC, features = c("FOXP3"), pt.size = 1, cols = c("grey","green4"), split.by = "predicted.celltype")
